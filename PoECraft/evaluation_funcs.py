...
pdps_dict = {"local_minimum_added_physical_damage" : [], "local_maximum_added_physical_damage" : [],
                  "physical_damage_+%" : [], "local_attack_speed_+%" : [], "local_item_quality_+" : []}

crafts_affixes = {"LocalPhysicalDamagePercent": "prefix",
...
pdps_stats = {"local_minimum_added_physical_damage", "local_maximum_added_physical_damage",
 "local_physical_damage_+%", "local_attack_speed_+%", "local_item_quality_+"}
pdps_groups = {"LocalPhysicalDamagePercent": "prefix",
          "PhysicalDamage": "prefix",
          "IncreasedAttackSpeed": "suffix",
          "CriticalStrikeChanceIncrease": "suffix",
          "": None}
es_groups = {"DefencesPercent": "prefix",
...
def average_pdps(pdps_dict, base_flat = 46, base_qual = 20, base_ias = 1.6):
    ...
    average_flat = (np.sum(pdps_dict["local_minimum_added_physical_damage"]) + np.sum(pdps_dict["local_maximum_added_physical_damage"]))/4.0
    ...
    crafts = {"LocalPhysicalDamagePercent" : flat * (qual + 124.5) * ias * base_ias / 100**2,
     "PhysicalDamage" : (flat + 43) * qual * ias * base_ias / 100**2,
    "IncreasedAttackSpeed" : max(flat * qual * (ias + 18)* base_ias / 100**2,flat * (qual + 15.5)* (ias + 14.5) * base_ias / 100**2),
    "CriticalStrikeChanceIncrease" : flat * (qual + 15.5) * ias * base_ias / 100**2,
...
def average_es(item):
    ...
    crafts = {"DefencesPercent" : flat * (qual + 65) / 100,
