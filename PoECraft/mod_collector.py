...
def _get_positive_spawn_weight_tags(affix, starting_tags) -> set:
    ...
    raise ValueError(
        "default should be in starting_tags and in the weights for this affix"
    )
...
def generate_all_possible_affixes_and_tags(starting_tags, mod_pool):
    ...
    while added_new_affixes:
        ...
        for key, affix in mod_pool.items():
            ...
            (
                pos_spawn_weights_tags,
                relevant_start_tag,
            ) = _get_positive_spawn_weight_tags(affix, starting_tags)
            relevant_start_tags.add(relevant_start_tag)

            pos_spawn_weights_from_current_pool = pos_spawn_weights_tags.intersection(
                current_tag_pool
            )
...
def collect_mods_and_tags(starting_tags, appended_mod_dictionary, domains, ilvl):
    ...
    (
        ...
        added_tags,
        affixes,
    ) = generate_all_possible_affixes_and_tags(starting_tags, mod_pool)
